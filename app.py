...
def build_verdict(url, page_text, reputation_text, wire_ai_result, mode):
    ...
    if is_scam:
        ...
        flag_lines = "\n".join(
            f"   {i+1}. {f['label']}" for i, f in enumerate([x for x in flags if x['type'] == 'threat'][:5])
        )
        complaint = (
            f"To,\n"
            f"The Cyber Crime Cell,\n"
            f"National Cyber Crime Reporting Portal (cybercrime.gov.in)\n\n"
            f"Date: {today}\n\n"
            f"Subject: Complaint Regarding Online Fraud / Cyber Scam\n\n"
            f"Respected Sir/Madam,\n\n"
            f"I am writing to formally report a suspected online fraud/scam that I encountered. "
            f"The details of the fraudulent content are as follows:\n\n"
            f"1. FRAUDULENT LINK / CONTENT:\n"
            f"   {url}\n\n"
            f"2. NATURE OF FRAUD:\n"
            f"   {stype}\n\n"
            f"3. FRAUD INDICATORS DETECTED:\n"
            f"{flag_lines}\n\n"
            f"4. AI ANALYSIS SUMMARY:\n"
            f"   {reason}"
            f"{chr(10) + '   Fake urgency/pressure tactics were also detected.' if has_urgency else ''}\n\n"
            f"5. TRUST SCORE ASSIGNED BY ScamShield AI: {score}/100 (High Risk)\n\n"
            f"I request you to kindly investigate this matter, take appropriate legal action against "
            f"the perpetrators, and block the above mentioned fraudulent link/content to prevent "
            f"further victims.\n\n"
            f"I am willing to provide any additional information required for the investigation.\n\n"
            f"Thanking you,\n\n"
            f"[Your Full Name]\n"
            f"[Your Mobile Number]\n"
            f"[Your Email Address]\n"
            f"[Your Address]\n\n"
            f"Generated by ScamShield AI — cybercrime.gov.in"
        )
